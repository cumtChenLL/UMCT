...
deg_optimizer = optim.Adam(le_degrader.parameters(), lr = 1e-4, betas=(0.9, 0.999), eps=1e-8,weight_decay= 0)
...
def one_epoch_train(args, data_loader, generator, degrader, epoch):
    ...
    for batch, (low, high, name) in enumerate(cur_dataloader):
        ...
        le_penalty = le_loss(deg_img, low) 
...
#    
#torch.save(le_degrader.state_dict(), "pre_model/le_degradation_{}.pt".format(epoch))
...
def le_test(args, dataloader, degrader, epoch):
    psnr_deg = []
    degrader.eval()
    with torch.no_grad():
        for batch, (low, high, name) in enumerate(dataloader):
            ...
            low, high = prepare(low, high)
            deg_img = degrader(high)
            #tensor_save_rgbimage(255*deg_img[0], "rec_"+str(name[0]), cuda=True)
...
le_degrader.load_state_dict(torch.load("pre_model/le_degradation_299.pt"))
le_test(args,test_LEdataLoader, le_degrader,1)
