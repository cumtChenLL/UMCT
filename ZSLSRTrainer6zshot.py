...
for epoch in range(epoches):
    for batch,(imgdata, name) in enumerate(cur_dataloader):
        ...
        if True: # zero-shot
            ...
            if False: # online
                ...
                rec_img_grad = gradcomput(rec_img)
                low_sr_g_grad = gradcomput(low_sr_g)
                # mask_rec_img_grad = rec_img_grad>0.5
                zssr_sr_grad = 1.0 * zshot_loss(rec_img_grad, low_sr_g_grad.detach())
                zssr_sr_light = cac_loss(rec_img, low_le_g.detach())
    ...
    with torch.no_grad():
        demo_lr = os.path.join(lr_datasetdir, 'r0603031et.png')
        ...
        imageio.imsave('demo_rec6zshot_Synthr0603031et.png', demo_rec)
    
    if epoch%100==1:
        torch.save(zssr_generator.state_dict(), "pre_model/zssr6_zshot_synth_latest.pt")
