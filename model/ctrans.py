...
def reconstruct(x, size_en):
    ...
    x = F.interpolate(x, size=size_en)
    return x
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]):
        ...
        self.embeddings_1 = Channel_Embeddings(config,self.patchSize[0], img_size=self.img_size[0], in_channels=channel_num[0])
        self.embeddings_2 = Channel_Embeddings(config,self.patchSize[1], img_size=self.img_size[1], in_channels=channel_num[1])
        self.embeddings_3 = Channel_Embeddings(config,self.patchSize[2], img_size=self.img_size[2], in_channels=channel_num[2])
        self.embeddings_4 = Channel_Embeddings(config,self.patchSize[3], img_size=self.img_size[3], in_channels=channel_num[3])
    ...
    def forward(self,en1,en2,en3,en4):
        ...
        ##downsampling size[H W] -> [96,96]
        size_en1 = en1.shape[-2:] if en1 is not None else None
        size_en2 = en2.shape[-2:] if en2 is not None else None
        size_en3 = en3.shape[-2:] if en3 is not None else None
        size_en4 = en4.shape[-2:] if en4 is not None else None
        #size_en = [size_en1, size_en2, size_en3, size_en4]
        en1_d = F.interpolate(en1, size=[self.img_size[0], self.img_size[0]]) if en1 is not None else None
        en2_d = F.interpolate(en2, size=[self.img_size[1], self.img_size[1]]) if en2 is not None else None
        en3_d = F.interpolate(en3, size=[self.img_size[2], self.img_size[2]]) if en3 is not None else None
        en4_d = F.interpolate(en4, size=[self.img_size[3], self.img_size[3]]) if en4 is not None else None

        emb1 = self.embeddings_1(en1_d)
        emb2 = self.embeddings_2(en2_d)
        emb3 = self.embeddings_3(en3_d)
        emb4 = self.embeddings_4(en4_d)

        encoded1, encoded2, encoded3, encoded4, attn_weights = self.encoder(emb1,emb2,emb3,emb4)  # (B, n_patch, hidden)
        encoded1 = reconstruct(encoded1, size_en1)
        x1 = self.reconstruct_1(encoded1) if en1 is not None else None
        encoded2 = reconstruct(encoded2, size_en2)
        x2 = self.reconstruct_2(encoded2) if en2 is not None else None
        encoded3 = reconstruct(encoded3, size_en3)
        x3 = self.reconstruct_3(encoded3) if en3 is not None else None
        encoded4 = reconstruct(encoded4, size_en4)
